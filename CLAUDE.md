...
- `jitcu/core/` — backend package. `cuda.py::load_cuda_ops` and `ascend.py::load_ascend_ops` each accept either a path list or a raw source string (written to `<build_dir>/<name>.cu` or `.cpp`), build the compiler command, hash the sources + output `.so` to decide if recompile is needed, and return a `Library`. `common.py` holds the shared `JITCULogger` + `hash_files` helper. `externals.py` holds the third-party-library registry consumed by the `external_libs=` kwarg of `load_cuda_ops` (see below). The Ascend path builds for the device via `bisheng` (`-xcce`, `--npu-arch=...`).
- `jitcu/library.py` — `Library` dlopens the `.so`, wires each exported symbol's `argtypes` from its `func_specs` ABI string, and wraps the call so Python-side torch tensors are converted to a C `Tensor` struct and the current CUDA/NPU stream is injected as arg 0. `func_specs` is a `dict[str, str]` mapping function name → ABI string; `_load` looks each name up via `getattr` and, if `dlsym` fails, raises a `RuntimeError` listing the symbols actually exported (via `_exported_functions`, which shells out to `nm -D` and drops mangled C++ names). **ABI contract**: every exported function must be `extern "C"` with signature `void f(<stream_t>, Tensor&, ..., <scalars>)` — the stream is implicit in the Python call, not listed in the ABI string. Currently supported ABI codes: `t` (tensor pointer), `i32`, `i64`, `f32`, `f64`.
...
### Adding a new scalar type to the `func_specs` ABI codes

Extend `Library.type_mapping` in `jitcu/library.py` with the matching ctypes type. Scalar arguments already pass through the CUDA/NPU wrappers unchanged; ctypes converts them according to `argtypes`. Supported floating-point scalar codes are `f32` (`float`) and `f64` (`double`). If adding a new data type carried in tensors, also keep `jitcu/data/include/jitcu/tensor.h::DataType` aligned with `library.py::Tensor.Dtype`. `tests/test_library.py` checks the real host ABI for both wrappers without requiring accelerator hardware.
...
- First positional arg of every exported C function is the stream (`cudaStream_t` / `aclrtStream`) — but it is **not** listed in the `func_specs` ABI string and **not** passed from Python; the wrapper injects `torch.cuda.current_stream()` / `torch.npu.current_stream()`.
